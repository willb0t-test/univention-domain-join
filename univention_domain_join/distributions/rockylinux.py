...
from univention_domain_join.join_steps.rocky_login_manager_configurator import RockyLoginManagerConfigurator
from univention_domain_join.join_steps.rocky_pam_configurator import RockyPamConfigurator
from univention_domain_join.join_steps.rocky_sssd_configurator import RockySssdConfigurator
...
class RockyLinuxJoiner(AbstractJoiner):
    ...
    def check_if_join_is_possible_without_problems(self) -> None:
        if not self.skip_login_manager and RockyLoginManagerConfigurator().configuration_conflicts():
    ...
    def create_backup_of_config_files(self) -> None:
        ...
        RockySssdConfigurator().backup(backup_dir)
        RockyPamConfigurator().backup(backup_dir)
        if not self.skip_login_manager:
            RockyLoginManagerConfigurator().backup(backup_dir)
    ...
    def join_domain(self) -> None:
        try:
            ...
            RockySssdConfigurator().setup_sssd(self.dc_ip, self.ldap_master, self.ldap_server_name, self.admin_username, self.admin_pw, self.ldap_base, self.kerberos_realm, admin_dn, is_samba_dc)
            RockyPamConfigurator().setup_pam()
            if not self.skip_login_manager:
                RockyLoginManagerConfigurator().enable_login_with_foreign_usernames()
