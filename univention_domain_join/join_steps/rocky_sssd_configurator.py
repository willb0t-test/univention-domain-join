...
class RockySssdConfigurator(ConflictChecker):
    ...
    def configure_sssd(self) -> None:
        userinfo_logger.info('Configuring SSSD for Rocky Linux')
        
        # On Rocky Linux, we don't use pam-auth-update as it doesn't exist
        # Instead, home directory creation is handled by authselect in the PAM configurator
        # Just ensure SSSD is enabled to start at boot
        try:
            subprocess.check_output(
                ['systemctl', 'enable', 'sssd'],
                stderr=subprocess.STDOUT
            )
            userinfo_logger.info('Enabled SSSD service to start at boot')
        except subprocess.CalledProcessError as e:
            userinfo_logger.warn(f'Could not enable SSSD service: {e.output.decode()}')

    @execute_as_root
    def restart_sssd(self) -> None:
        userinfo_logger.info('Restarting SSSD service on Rocky Linux')

        try:
            # Stop SSSD first in case it's running
            subprocess.run(['systemctl', 'stop', 'sssd'], 
            ...
            userinfo_logger.info('SSSD service started successfully')
            
            # Check status
            result = subprocess.run(['systemctl', 'is-active', 'sssd'], 
                                  capture_output=True, text=True)
            if result.returncode == 0 and 'active' in result.stdout:
                userinfo_logger.info('SSSD is running and active')
            else:
                userinfo_logger.warn('SSSD may not be running properly')
                
        except subprocess.CalledProcessError as e:
            userinfo_logger.error(f'Failed to restart SSSD: {e.output.decode()}')
            raise
